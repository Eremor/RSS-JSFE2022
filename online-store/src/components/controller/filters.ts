...
export class Filters {
  ...
  private sortFilter = (items: ICard[]): void => {
    switch (filters.sort) {
      case 'price-lowest':
        items.sort((a: ICard, b: ICard): number => (a.price > b.price ? 1 : -1));
        break;
      case 'price-highest':
        items.sort((a: ICard, b: ICard): number => (a.price > b.price ? -1 : 1));
        break;
      case 'desc-a':
        items.sort((a: ICard, b: ICard): number => (a.name > b.name ? 1 : -1));
        break;
      case 'desc-z':
        items.sort((a: ICard, b: ICard): number => (a.name > b.name ? -1 : 1));
        break;
      case 'newer':
        items.sort((a: ICard, b: ICard): number => (a.year > b.year ? 1 : -1));
        break;
      case 'older':
        items.sort((a: ICard, b: ICard): number => (a.year > b.year ? -1 : 1));
        break;
  ...
  private searchFilter = (items: ICard[]): void => {
    this.data = [...items.filter((item: ICard): boolean => item.name.includes(filters.search.toLowerCase()))];
  };

  private categoryFilter = (items: ICard[]): void => {
    this.data = [
      ...items.filter((item: ICard): ICard | boolean =>
        filters.category === '' ? item : item.category === filters.category
      ),
    ];
  };
  ...
  private companyFilter = (items: ICard[]): void => {
    this.data = [
      ...items.filter((item: ICard): ICard | boolean =>
        filters.company === '' ? item : item.company === filters.company
      ),
    ];
  };
